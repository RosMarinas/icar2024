    ...
    bool farmlandDetection(TrackRecognition &track, vector<PredictResult> predict)
    {
        ...
        case FarmlandStep::Enter: //[02] 开进进入
        {
            ...
            if (conesEdgeLeft.size() >= conesEdgeRight.size() && conesEdgeLeft.size() >= 2) // 左边补右边
            {
                ...
                POINT endPoint = conesEdgeLeft.back();
            ...
            else if (conesEdgeLeft.size() < conesEdgeRight.size() && conesEdgeRight.size() >= 2) // 右边补左边
            {
                ...
                POINT endPoint = conesEdgeRight.back();
                vector<POINT> input = {startPoint, midPoint, endPoint};
                vector<POINT> repair = Bezier(0.05, input);
                conesEdgeRight.clear();
        ...
        case FarmlandStep::Cruise: //[03] 完全驶入农田
        {
            ...
            for (int i = 0; i < pointEdgeDet.size(); i++) // 第一行边缘点搜索
            {
                if (pointEdgeDet[i].y < COLSIMAGE / 2 && pointEdgeDet[i].x > conesEdgeLeft[0].x && pointEdgeDet[i].x > ROWSIMAGE / 2 && pointEdgeDet[i].x > ROWSIMAGE / 2)
                    conesEdgeLeft[0] = pointEdgeDet[i];

                if (pointEdgeDet[i].y > COLSIMAGE / 2 && pointEdgeDet[i].x > conesEdgeRight[0].x && pointEdgeDet[i].x > ROWSIMAGE / 2 && pointEdgeDet[i].x > ROWSIMAGE / 2)
                    conesEdgeRight[0] = pointEdgeDet[i];
            }
...
private:
    ...
    void searchConesEdge(vector<POINT> cones)
    {
        ...
        std::sort(pointsCones.begin(), pointsCones.end(), bmp);
        if (conesEdgeLeft.size() == 0 && conesEdgeRight.size() == 0) // 未搜索到起点
            return;
        
        int ldyl = 0, ldyr = 0;
        float lddl = 60, lddr = 60;
        ...
        while (pointsCones.size() > 0)
        {
            ...
            for (int i = 0; i < pointsCones.size(); i++) // 搜索左边缘
            {
                ...
                {
                    if(distance(conesEdgeLeft.back(), pointsCones[i]) < lddl && conesEdgeLeft[0].x != pointsCones[i].x && conesEdgeRight[0].x != pointsCones[i].x && pointsCones[i].y < COLSIMAGE / 2)
                    {
                        if(!ldyl){
                            index = i;
                            lddl = distance(conesEdgeLeft.back(), pointsCones[i]);
                            ldyl = pointsCones[i].y - conesEdgeLeft.back().y;
                        }
                        else{
                            if(ldyl < 0 && pointsCones[i].y < conesEdgeLeft.back().y)
                            {
                                index = i;
                                lddl = distance(conesEdgeLeft.back(), pointsCones[i]);
                                ldyl = pointsCones[i].y - conesEdgeLeft.back().y;
                            }
                            else if(ldyl > 0 && pointsCones[i].y > conesEdgeLeft.back().y)
                            {
                                index = i;
                                lddl = distance(conesEdgeLeft.back(), pointsCones[i]);
                                ldyl = pointsCones[i].y - conesEdgeLeft.back().y;
            ...
            for (int i = 0; i < pointsCones.size(); i++) // 搜索右边缘
            {
                ...
                {
                    if(distance(conesEdgeRight.back(), pointsCones[i]) < lddr&& conesEdgeRight[0].x != pointsCones[i].x && conesEdgeLeft[0].x != pointsCones[i].x && pointsCones[i].y > COLSIMAGE / 2)
                    {
                        if(!ldyr){
                            index = i;
                            lddr = distance(conesEdgeRight.back(), pointsCones[i]);
                            ldyr = conesEdgeRight[i].y - conesEdgeRight.back().y;
                        }
                        else{
                            if(ldyr < 0 && pointsCones[i].y < conesEdgeRight.back().y)
                            {
                                index = i;
                                lddr = distance(conesEdgeRight.back(), pointsCones[i]);
                                ldyr = pointsCones[i].y - conesEdgeRight.back().y;
                            }
                            else if(ldyr > 0 && pointsCones[i].y > conesEdgeRight.back().y)
                            {
                                index = i;
                                lddr = distance(conesEdgeRight.back(), pointsCones[i]);
                                ldyr = pointsCones[i].y - conesEdgeRight.back().y;
