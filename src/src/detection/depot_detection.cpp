...
public:
  ...
  bool depotDetection(TrackRecognition &track, vector<PredictResult> predict) {
    ...
    case DepotStep::DepotEnable: //[02] 维修厂使能
    {
      ...
      searchCones(predict);
      POINT lastpoint(_pointNearCone.x, _pointNearCone.y);
      _pointNearCone = searchNearestCone(track.pointsEdgeLeft, pointEdgeDet); // 搜索右下锥桶
      printf("(%d %d)\n", _pointNearCone.x, _pointNearCone.y);
      if (lastpoint.x - _pointNearCone.x > 100)
      {
        depotStep = DepotStep::DepotEnter;
        counterRec = 0;
        counterSession = 0;
      }
      // if (_pointNearCone.x >
    ...
    case DepotStep::DepotCruise: //[04] 巡航使能
    {
      ...
      if (heighestCone.x > 0 && heighestCone.y > 0) {
        ...
        if (levelCones > ROWSIMAGE * depotStop || levelCones == 0) {
          counterRec++;
          if (counterRec > 3) {
            depotStep = DepotStep::DepotStop; // 停车使能
            counterRec = 0;
