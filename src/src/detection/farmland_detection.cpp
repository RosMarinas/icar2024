    ...
    bool farmlandDetection(TrackRecognition &track, vector<PredictResult> predict)
    {
        ...
        case FarmlandStep::Enter: //[02] 开进进入
        {
            ...
            else // 初始左右锥桶搜索
            {
                ...
                {
                    if (pointEdgeDet[i].y < COLSIMAGE / 2 && pointEdgeDet[i].x > conesEdgeLeft[0].x && pointEdgeDet[i].x > ROWSIMAGE / 2)
                        conesEdgeLeft[0] = pointEdgeDet[i];

                    if (pointEdgeDet[i].y > COLSIMAGE / 2 && pointEdgeDet[i].x > conesEdgeRight[0].x && pointEdgeDet[i].x > ROWSIMAGE / 2)
